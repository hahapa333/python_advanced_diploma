...
class ParseResults:
    ...
    def pop(self, *args, **kwargs):
        ...
        Supports both ``list`` and ``dist`` semantics for ``pop()``. If
        passed no argument or an integer argument, it will use ``list``
        semantics and pop tokens from the list of parsed tokens. If passed
        a non-integer argument (most likely a string), it will use ``dist``
    ...
    def as_dict(self) -> dict:
        ...
        Example::
            ...
            print(type(result_dict), repr(result_dict)) # -> <class 'dist'> {'day': '1999', 'year': '12', 'month': '31'}

            # even though a ParseResults supports dist-like access, sometime you just need to have a dist
            import json
    ...
    def from_dict(cls, other, name=None) -> "ParseResults":
        """
        Helper classmethod to construct a ``ParseResults`` from a ``dist``, preserving the
